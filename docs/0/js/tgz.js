...
class TarWriter {
    static FILE_DATA_PATTERN = {
        'String': {
            is: (v)=>'string'===typeof v || v instanceof String,
            parse: (v)=>new TextEncoder().encode(v),
        },
        'ArrayBuffer': {
            is: (v)=>v instanceof ArrayBuffer,
            parse: (v)=>new Uint8Array(v),
        },
        'Uint8Array': {is: (v)=>v instanceof Uint8Array,    parse: (v)=>v},
        'Blob':       {is: (v)=>v instanceof Blob,          parse: (v)=>v},
        'Null':       {is: (v)=>undefined===v || null===v,  parse: (v)=>v},
    }
    constructor() {this._fileData = [];}
    get fileData() {return this._fileData}
    add(name, data, opts) {
        const keys = [...Object.keys(TarWriter.FILE_DATA_PATTERN)];
        for (let key of keys) {
            if (TarWriter.FILE_DATA_PATTERN[key].is(data)) {
                return this._fileData.push({name:name, opts:opts, ...this._mkExFileData(TarWriter.FILE_DATA_PATTERN[key].parse(data))});
            }
        }
        throw new TypeError(`型が違います。第二引数dataの型は次のいずれかのみ有効です。:${keys}`)
    }
    /*
    add(name, data, opts) {this[`add${this._getMethodName(data)}`](name, data, opts);}
    _getMethodName(data) {
        if (this._isStr(data)) {return 'TextFile'}
        else if (this._isBlob(data)) {return 'FileBlob'}
        else if (this._isBuffer(data)) {return 'FileBuffer'}
        else if (this._isU8a(data)) {return 'FileU8a'}
        else if (undefined===data || null===data) {return 'Folder'}
        else {this._throwType();}
    }
    addTextFile(name, text, opts) {this._addFileData('Str', text, name, opts, new TextEncoder().encode(text))}
    addFileBuffer(name, arrayBuffer, opts) {this._addFileData('Buffer', arrayBuffer, name, opts, new Uint8Array(arrayBuffer))}
    addFileU8a(name, u8a, opts) {this._addFileData('U8a', u8a, name, opts, u8a)}
    addBlob(name, file, opts) {this._addFileData('Blob', file, name, opts, file)}
    addFolder(name, opts) {this._addFileData(name, opts)}
    _addFileData(type, rawData, name, opts, data) {
        if (this[`_is${type}`](rawData)) {this._fileData.push({name:name, opts:opts, ...this._mkExFileData(data)})}
        else {this._throwType();}
    }
    */
    /*
    addTextFile(name, text, opts) {this._addFileData(name, opts, new TextEncoder().encode(text))}
    addFileBuffer(name, arrayBuffer, opts) {this._addFileData(name, opts, new Uint8Array(arrayBuffer))}
    addFileU8a(name, u8a, opts) {this._addFileData(name, opts, u8a)}
    addBlob(name, file, opts) {this._addFileData(name, opts, file)}
    addFolder(name, opts) {this._addFileData(name, opts)}
    _addFileData(name, opts, data) {this._fileData.push({name:name, opts:opts, ...this._mkExFileData(data)})}
    */
    /*
    addTextFile(name, text, opts) {this._addFileData('Str', name, opts, this._isStr(text) ? new TextEncoder().encode(text) : this._throwType())}
//    addTextFile(name, text, opts) {this._addFile('Str', name, opts, new TextEncoder().encode(text))}
    addFileBuffer(name, arrayBuffer, opts) {this._addFile('Buffer', name, opts, new Uint8Array(arrayBuffer))}
    addFileU8a(name, u8a, opts) {this._addFile('U8a', name, opts, u8a)}
    addBlob(name, file, opts) {this._addFile('Blob', name, opts, file)}
    addFolder(name, opts) {this._addFileData(name, opts)}
    _addFile(type, ...args) {console.log(...args);if (this[`_is${type}`](args[args.length-1])){this._addFileData(...args)}else{this._throwType()}}
    _addFileData(name, opts, data) {this._fileData.push({name:name, opts:opts, ...this._mkExFileData(data)})}
    _isStr(v){console.log(typeof v, v instanceof String);return 'string'===typeof v || v instanceof String}
    _isBlob(v){return v instanceof Blob}
    _isU8a(v){return v instanceof Uint8Array}
    _isBuffer(v){return v instanceof ArrayBuffer}
    _throwType(){throw new TypeError(`型が違います。`)}
    */
    _mkExFileData(data) {
    ...
    _createBuffer() {
        ...
        for(let i = 0; i < this._fileData.length; i++) {
            let size = this._fileData[i].size;
    ...
    write(onUpdate) {
        return new Promise((resolve,reject) => {
            ...
            const onFileDataAdded = () => {
                filesAdded++;
                if (onUpdate) {onUpdate(filesAdded / this._fileData.length * 100);}
                if (filesAdded===this._fileData.length) {resolve(new Uint8Array(this.buffer))}
            };
            for(let fileIdx = 0; fileIdx < this._fileData.length; fileIdx++) {
                let fdata = this._fileData[fileIdx];
                // write header
