...
class TarReader {
    ...
    *_getFiles() {
    ...
    get fileData() {return this.fileInfo.map(info=>({...info, opts:this._getFileOpt(info.header_offset)}))}
    _getFileOpt(header_offset) { return {
        mode: this._readFileMode(header_offset),
        uid: this.__readFileUid(header_offset),
        gid: this._readFileGid(header_offset),
        mtime: this._readFileMtime(header_offset),
        user: this._readFileUser(header_offset),
        group: this._readFileGroup(header_offset),
    } }
    async from(data) {
        if (data instanceof Blob) {return await this.fromFile(data)}
        else if (data instanceof ArrayBuffer) {return this.fromBuffer(data)}
        else if (data instanceof Uint8Array) {return this.fromBuffer(data.buffer)}
        else {throw new TypeError(`引数の型が不正です。Blob(File), ArrayBuffer, Uint8Arrayのみ有効です。`)}
    }
    fromFile(file) {
...
class TarWriter {
    static FILE_DATA_PATTERN = {
        'String': {
            ...
            parse: (v)=>new TextEncoder().encode(v), // Uint8Array
        },
        'ArrayBuffer': {
...
//[Blob, Response].map(C=>C.bytes ??= async()=>{return new Uint8Array(await this.arrayBuffer());});// polyfill
//[Blob, Response].map(C=>C.bytes ??= async function() {return new Uint8Array(await this.arrayBuffer());});// polyfill
...
class TgzWriter extends TarWriter {
    constructor() {super()}
    async download(fileName) {console.log(fileName);return await super.download(fileName ?? 'some.tgz')}
